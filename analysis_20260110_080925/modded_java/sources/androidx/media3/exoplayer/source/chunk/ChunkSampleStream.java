...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk>, Loader.ReleaseCallback {
    ...
    public boolean continueLoading(LoadingInfo loadingInfo) {
        ...
        BaseMediaChunk baseMediaChunk = this.nextChunkHolder.chunk;
        ...
        this.loadingChunk = baseMediaChunk;
        if (isMediaChunk(baseMediaChunk)) {
            ...
            baseMediaChunk2.init(this.chunkOutput);
            this.mediaChunks.add(baseMediaChunk2);
        } else if (baseMediaChunk instanceof InitializationChunk) {
            ((InitializationChunk) baseMediaChunk).init(this.chunkOutput);
        }
        this.mediaSourceEventDispatcher.loadStarted(new LoadEventInfo(((Chunk) baseMediaChunk).loadTaskId, ((Chunk) baseMediaChunk).dataSpec, this.loader.startLoading(baseMediaChunk, this, this.loadErrorHandlingPolicy.getMinimumLoadableRetryCount(((Chunk) baseMediaChunk).type))), ((Chunk) baseMediaChunk).type, this.primaryTrackType, ((Chunk) baseMediaChunk).trackFormat, ((Chunk) baseMediaChunk).trackSelectionReason, ((Chunk) baseMediaChunk).trackSelectionData, ((Chunk) baseMediaChunk).startTimeUs, ((Chunk) baseMediaChunk).endTimeUs);
        return true;
    }
    ...
    public void reevaluateBuffer(long j) {
        ...
        if (this.loader.isLoading()) {
            BaseMediaChunk baseMediaChunk = (Chunk) Assertions.checkNotNull(this.loadingChunk);
            if (!(isMediaChunk(baseMediaChunk) && haveReadFromMediaChunk(this.mediaChunks.size() - 1)) && this.chunkSource.shouldCancelLoad(j, baseMediaChunk, this.readOnlyMediaChunks)) {
                this.loader.cancelLoading();
                if (isMediaChunk(baseMediaChunk)) {
                    this.canceledMediaChunk = baseMediaChunk;
                    return;
                }
                return;
            }
