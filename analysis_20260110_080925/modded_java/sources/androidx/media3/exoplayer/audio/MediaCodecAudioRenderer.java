...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    protected int supportsFormat(MediaCodecSelector mediaCodecSelector, Format format) throws MediaCodecUtil.DecoderQueryException {
        ...
        return RendererCapabilities.create(z2 ? 4 : 3, (z2 && mediaCodecInfo.isSeamlessAdaptationSupported(format)) ? 16 : 8, i2, mediaCodecInfo.hardwareAccelerated ? 64 : 0, z ? RendererCapabilities.DECODER_SUPPORT_PRIMARY : 0, i);
    }
    ...
    private int getAudioOffloadSupport(Format format) {
        ...
        int i = RendererCapabilities.AUDIO_OFFLOAD_SUPPORTED;
        if (formatOffloadSupport.isGaplessSupported) {
            i = 1536;
        }
        return formatOffloadSupport.isSpeedChangeSupported ? i | RendererCapabilities.AUDIO_OFFLOAD_SPEED_CHANGE_SUPPORTED : i;
    }
    ...
    protected boolean shouldUseBypass(Format format) {
        if (getConfiguration().offloadModePreferred != 0) {
            ...
            if ((audioOffloadSupport & RendererCapabilities.AUDIO_OFFLOAD_SUPPORTED) != 0) {
                if (getConfiguration().offloadModePreferred == 2 || (audioOffloadSupport & RendererCapabilities.AUDIO_OFFLOAD_GAPLESS_SUPPORTED) != 0) {
                    return true;
                }
    ...
    protected DecoderReuseEvaluation canReuseCodec(MediaCodecInfo mediaCodecInfo, Format format, Format format2) {
        ...
        if (isBypassPossible(format2)) {
            i |= Connections.MAX_BYTES_DATA_SIZE;
        }
    ...
    /* JADX INFO: Thrown type has an unknown type hierarchy: androidx.media3.exoplayer.ExoPlaybackException */
