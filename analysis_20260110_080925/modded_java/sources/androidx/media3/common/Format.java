...
public final class Format implements Bundleable {
    ...
    public static String toLogString(Format format) {
        ...
        if (format.roleFlags != 0) {
            ...
            if ((format.roleFlags & RendererCapabilities.DECODER_SUPPORT_PRIMARY) != 0) {
                arrayList2.add("subtitle");
            }
            if ((format.roleFlags & 256) != 0) {
                arrayList2.add("sign");
            }
            if ((format.roleFlags & RendererCapabilities.AUDIO_OFFLOAD_SUPPORTED) != 0) {
                arrayList2.add("describes-video");
            }
            if ((format.roleFlags & RendererCapabilities.AUDIO_OFFLOAD_GAPLESS_SUPPORTED) != 0) {
                arrayList2.add("describes-music");
            }
            if ((format.roleFlags & RendererCapabilities.AUDIO_OFFLOAD_SPEED_CHANGE_SUPPORTED) != 0) {
            ...
            if ((format.roleFlags & JsonLexerKt.BATCH_SIZE) != 0) {
    ...
    private static String keyForInitializationData(int i) {
        return FIELD_INITIALIZATION_DATA + EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR + Integer.toString(i, 36);
    }
}
