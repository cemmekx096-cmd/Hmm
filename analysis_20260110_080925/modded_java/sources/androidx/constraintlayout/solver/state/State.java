...
public class State {
    ...
    public GuidelineReference guideline(Object obj, int i) {
        Reference guidelineReference = this.mReferences.get(obj);
        if (guidelineReference == null) {
    ...
    public BarrierReference barrier(Object obj, Direction direction) {
        BarrierReference barrierReferenceHelper = helper(obj, Helper.BARRIER);
        barrierReferenceHelper.setBarrierDirection(direction);
        return barrierReferenceHelper;
    }

    public VerticalChainReference verticalChain(Object... objArr) {
        VerticalChainReference verticalChainReferenceHelper = helper(null, Helper.VERTICAL_CHAIN);
        verticalChainReferenceHelper.add(objArr);
        return verticalChainReferenceHelper;
    }

    public HorizontalChainReference horizontalChain(Object... objArr) {
        HorizontalChainReference horizontalChainReferenceHelper = helper(null, Helper.HORIZONTAL_CHAIN);
        horizontalChainReferenceHelper.add(objArr);
        return horizontalChainReferenceHelper;
    }

    public AlignHorizontallyReference centerHorizontally(Object... objArr) {
        AlignHorizontallyReference alignHorizontallyReferenceHelper = helper(null, Helper.ALIGN_HORIZONTALLY);
        alignHorizontallyReferenceHelper.add(objArr);
        return alignHorizontallyReferenceHelper;
    }

    public AlignVerticallyReference centerVertically(Object... objArr) {
        AlignVerticallyReference alignVerticallyReferenceHelper = helper(null, Helper.ALIGN_VERTICALLY);
        alignVerticallyReferenceHelper.add(objArr);
        return alignVerticallyReferenceHelper;
    }
    ...
    public void apply(ConstraintWidgetContainer constraintWidgetContainer) {
        ...
        for (Object obj : this.mHelperReferences.keySet()) {
            ...
            if (helperWidget != null) {
                ConstraintReference constraintReferenceConstraints = (Reference) this.mReferences.get(obj);
                if (constraintReferenceConstraints == null) {
                    constraintReferenceConstraints = constraints(obj);
                }
                constraintReferenceConstraints.setConstraintWidget(helperWidget);
            }
        }
        Iterator<Object> it = this.mReferences.keySet().iterator();
        while (it.hasNext()) {
            ConstraintReference constraintReference = (Reference) this.mReferences.get(it.next());
            if (constraintReference != this.mParent) {
                ConstraintWidget constraintWidget = constraintReference.getConstraintWidget();
                constraintWidget.setParent((ConstraintWidget) null);
                if (constraintReference instanceof GuidelineReference) {
                    constraintReference.apply();
                }
                constraintWidgetContainer.add(constraintWidget);
            } else {
                constraintReference.setConstraintWidget(constraintWidgetContainer);
            }
        }
        ...
        while (it2.hasNext()) {
            ...
            if (helperReference.getHelperWidget() != null) {
                Iterator it3 = helperReference.mReferences.iterator();
                while (it3.hasNext()) {
