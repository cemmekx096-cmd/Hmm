...
public class WriteTree {
    ...
    public boolean removeWrite(long j) {
        ...
        if (next.isOverwrite()) {
            ...
            Iterator it2 = next.getMerge().iterator();
            while (it2.hasNext()) {
                this.visibleWrites = this.visibleWrites.removeWrite(next.getPath().child((Path) ((Map.Entry) it2.next()).getKey()));
            }
        }
        return true;
    }
    ...
    public Node calcCompleteEventChildren(Path path, Node node) {
        ...
        while (it.hasNext()) {
            ...
            nodeEmpty = nodeEmpty.updateImmediateChild(namedNode2.getName(), compoundWriteChildCompoundWrite.childCompoundWrite(new Path(new ChildKey[]{namedNode2.getName()})).apply(namedNode2.getNode()));
        }
