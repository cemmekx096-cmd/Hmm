...
public abstract class PreferenceFragment extends Fragment implements PreferenceManager.OnPreferenceTreeClickListener, PreferenceManager.OnDisplayPreferenceDialogListener, PreferenceManager.OnNavigateToScreenListener, DialogPreference.TargetFragment {
    ...
    public void setPreferencesFromResource(int i, String str) {
        requirePreferenceManager();
        Preference preferenceInflateFromResource = this.mPreferenceManager.inflateFromResource(this.mStyledContext, i, (PreferenceScreen) null);
        if (str != null) {
            preferenceInflateFromResource = preferenceInflateFromResource.findPreference(str);
            if (!(preferenceInflateFromResource instanceof PreferenceScreen)) {
                throw new IllegalArgumentException("Preference object with key " + str + " is not a PreferenceScreen");
            }
        }
        setPreferenceScreen((PreferenceScreen) preferenceInflateFromResource);
    }
    ...
    public RecyclerView onCreateRecyclerView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        if (this.mStyledContext.getPackageManager().hasSystemFeature("android.hardware.type.automotive") && (recyclerViewFindViewById = viewGroup.findViewById(R.id.recycler_view)) != null) {
            return recyclerViewFindViewById;
        }
        RecyclerView recyclerViewInflate = layoutInflater.inflate(R.layout.preference_recyclerview, viewGroup, false);
        recyclerViewInflate.setLayoutManager(onCreateLayoutManager());
        recyclerViewInflate.setAccessibilityDelegateCompat(new PreferenceRecyclerViewAccessibilityDelegate(recyclerViewInflate));
        return recyclerViewInflate;
    }
    ...
    public void onDisplayPreferenceDialog(Preference preference) {
        EditTextPreferenceDialogFragment editTextPreferenceDialogFragmentNewInstance;
        ...
        if (!zOnPreferenceDisplayDialog && getFragmentManager().findFragmentByTag(DIALOG_FRAGMENT_TAG) == null) {
            if (preference instanceof EditTextPreference) {
                editTextPreferenceDialogFragmentNewInstance = EditTextPreferenceDialogFragment.newInstance(preference.getKey());
            } else if (preference instanceof ListPreference) {
                editTextPreferenceDialogFragmentNewInstance = ListPreferenceDialogFragment.newInstance(preference.getKey());
            } else if (preference instanceof MultiSelectListPreference) {
                editTextPreferenceDialogFragmentNewInstance = MultiSelectListPreferenceDialogFragment.newInstance(preference.getKey());
            } else {
                throw new IllegalArgumentException("Tried to display dialog for unknown preference type. Did you forget to override onDisplayPreferenceDialog()?");
            }
            editTextPreferenceDialogFragmentNewInstance.setTargetFragment(this, 0);
            editTextPreferenceDialogFragmentNewInstance.show(getFragmentManager(), DIALOG_FRAGMENT_TAG);
        }
    }
    ...
    private void scrollToPreferenceInternal(final Preference preference, final String str) {
        Runnable runnable = new Runnable() { // from class: androidx.preference.PreferenceFragment.3
            ...
            public void run() {
                ...
                PreferenceGroup.PreferencePositionCallback adapter = PreferenceFragment.this.mList.getAdapter();
                ...
                if (preference2 != null) {
                    preferenceAdapterPosition = adapter.getPreferenceAdapterPosition(preference2);
                } else {
                    preferenceAdapterPosition = adapter.getPreferenceAdapterPosition(str);
                }
    ...
    private static class ScrollToPreferenceObserver extends RecyclerView.AdapterDataObserver {
        ...
        private void scrollToPreference() {
            ...
            if (preference != null) {
                preferenceAdapterPosition = this.mAdapter.getPreferenceAdapterPosition(preference);
            } else {
                preferenceAdapterPosition = this.mAdapter.getPreferenceAdapterPosition(this.mKey);
            }
            if (preferenceAdapterPosition != -1) {
