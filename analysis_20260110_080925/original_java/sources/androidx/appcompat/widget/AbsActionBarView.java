...
abstract class AbsActionBarView extends ViewGroup {
    ...
    public boolean onHoverEvent(MotionEvent ev) {
        ...
        if (!this.mEatingHover) {
            boolean zOnHoverEvent = super.onHoverEvent(ev);
    ...
    public ViewPropertyAnimatorCompat setupAnimatorToVisibility(int visibility, long duration) {
        ...
        if (visibility == 0) {
            ...
            viewPropertyAnimatorCompatAlpha.setDuration(duration);
            viewPropertyAnimatorCompatAlpha.setListener(this.mVisAnimListener.withFinalVisibility(viewPropertyAnimatorCompatAlpha, visibility));
        ...
        viewPropertyAnimatorCompatAlpha2.setDuration(duration);
        viewPropertyAnimatorCompatAlpha2.setListener(this.mVisAnimListener.withFinalVisibility(viewPropertyAnimatorCompatAlpha2, visibility));
    ...
    protected int measureChildView(View child, int availableWidth, int childSpecHeight, int spacing) {
        child.measure(View.MeasureSpec.makeMeasureSpec(availableWidth, ExoMediaDrm.KeyRequest.REQUEST_TYPE_UNKNOWN), childSpecHeight);
    ...
    protected int positionChild(View child, int x, int y, int contentHeight, boolean reverse) {
        ...
        int i = y + ((contentHeight - measuredHeight) / 2);
        if (reverse) {
            child.layout(x - measuredWidth, i, x, measuredHeight + i);
        } else {
            child.layout(x, i, x + measuredWidth, measuredHeight + i);
        }
