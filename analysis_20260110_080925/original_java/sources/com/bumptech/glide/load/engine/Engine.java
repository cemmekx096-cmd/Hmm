...
public class Engine implements EngineJobListener, MemoryCache.ResourceRemovedListener, EngineResource.ResourceListener {
    ...
    private <R> LoadStatus waitForExistingOrStartNewJob(GlideContext glideContext, Object obj, Key key, int i, int i2, Class<?> cls, Class<R> cls2, Priority priority, DiskCacheStrategy diskCacheStrategy, Map<Class<?>, Transformation<?>> map, boolean z, boolean z2, Options options, boolean z3, boolean z4, boolean z5, boolean z6, ResourceCallback resourceCallback, Executor executor, EngineKey engineKey, long j) {
        ...
        DecodeJob.Callback<R> callbackBuild = this.engineJobFactory.build(engineKey, z3, z4, z5, z6);
        DecodeJob<R> decodeJobBuild = this.decodeJobFactory.build(glideContext, obj, engineKey, key, i, i2, cls, cls2, priority, diskCacheStrategy, map, z, z2, z6, options, callbackBuild);
        this.jobs.put(engineKey, callbackBuild);
        callbackBuild.addCallback(resourceCallback, executor);
        callbackBuild.start(decodeJobBuild);
        if (VERBOSE_IS_LOGGABLE) {
            logWithTimeAndKey("Started new load", j, engineKey);
        }
        return new LoadStatus(resourceCallback, callbackBuild);
    }
    ...
    private EngineResource<?> getEngineResourceFromCache(Key key) {
        EngineResource<?> engineResourceRemove = this.cache.remove(key);
        if (engineResourceRemove == null) {
            return null;
        }
        if (engineResourceRemove instanceof EngineResource) {
            return engineResourceRemove;
        }
        return new EngineResource<>(engineResourceRemove, true, true, key, this);
    }
