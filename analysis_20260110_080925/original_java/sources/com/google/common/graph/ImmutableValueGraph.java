...
public final class ImmutableValueGraph<N, V> extends StandardValueGraph<N, V> {
    ...
    private static <N, V> ImmutableMap<N, GraphConnections<N, V>> getNodeConnections(ValueGraph<N, V> valueGraph) {
        ...
        for (Object obj : valueGraph.nodes()) {
            builder.put(obj, connectionsOf(valueGraph, obj));
        }
        return builder.buildOrThrow();
    }

    private static <N, V> GraphConnections<N, V> connectionsOf(ValueGraph<N, V> valueGraph, N n) {
        ImmutableValueGraph$.ExternalSyntheticLambda0 externalSyntheticLambda0 = new ImmutableValueGraph$.ExternalSyntheticLambda0(valueGraph, n);
        if (valueGraph.isDirected()) {
            return DirectedGraphConnections.ofImmutable(n, valueGraph.incidentEdges(n), externalSyntheticLambda0);
        }
        return UndirectedGraphConnections.ofImmutable(Maps.asMap(valueGraph.adjacentNodes(n), externalSyntheticLambda0));
    }
